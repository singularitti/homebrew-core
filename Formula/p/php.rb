class Php < Formula
  ...
  stable do
    ...
    # Backport fix for curl 8.16 on macOS. The .patch file won't work with merge commit and audit rejects .diff
    # https://github.com/php/php-src/commit/8e1df69d34568c92d3920dc9fb22c490542d4fe0.diff
    patch :DATA
  end
  ...
  def install
    ...
    system "./configure", *args, *intl_arg
    system "make"
    system "make", "install"

    if OS.mac?
      # Build C++ extension modules with Clang
      system "./configure", *args, "--disable-opcache", "--enable-intl=shared"
  ...
  test do
    ...
    system bin/"php", "-r", "(new \\MessageFormatter('de_DE','{0,number,#,###.##} MB'))->format([12.5]);"
    begin
      port = free_port
      port_fpm = free_port
...
index 4b0391541f3b..7cd8795c936e 100644
